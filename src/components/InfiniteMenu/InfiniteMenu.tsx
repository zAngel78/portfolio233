...
export default function InfiniteMenu({ items = [], scale = 1.0, projects = [] }) {
    ...
    const handleActiveItem = index => {
      ...
      if (projects && projects.length > 0) {
        const project = projects.find(p =>
          (p.name && p.name === items[itemIndex]?.title) ||
          p.title === items[itemIndex]?.title
        );
  ...
  }, [items, scale, projects]);

  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <canvas id="infinite-grid-menu-canvas" ref={canvasRef} />

      {/* Viñetas laterales negras */}
      <div className={`vignette-left ${!isMoving && activeProject ? 'active' : 'inactive'}`} />
      <div className={`vignette-right ${!isMoving && activeProject ? 'active' : 'inactive'}`} />

      {/* Información del proyecto a la izquierda */}
      {activeProject && (
        <>
          <div className={`project-info-left ${isMoving ? 'inactive' : 'active'}`}>
            ...
            <h2 className="project-name">{activeProject.name || activeProject.title}</h2>
            {activeProject.role && (
          ...
          <div className={`project-info-right ${isMoving ? 'inactive' : 'active'}`}>
            <div className="project-technologies">
              <h3 className="tech-label">
                {activeProject.technologies.length === 1 ? 'Technology' : 'Tech Stack'}
              </h3>
              <div className="tech-list">
                {activeProject.technologies.slice(0, 6).map((tech, index) => (
            ...
            {activeProject.link && (
              ...
              >
                {activeProject.category === 'E-commerce' ? 'Visit Store' :
                 activeProject.category === 'Platform' ? 'Try Platform' :
                 activeProject.category === 'Tool' ? 'Use Tool' :
                 'Explore Project'} →
              </a>
            )}
          </div>
